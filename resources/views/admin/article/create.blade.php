...
@section('content')
    ...
    <form class="form-horizontal " action="{{ url('admin/article/store') }}" method="post" enctype="multipart/form-data">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tr>
                ...
                <td>
                    @foreach($tag as $v)
                        {{ $v['name'] }}<input class="yzh-icheck" type="checkbox" name="tag_ids[]" value="{{ $v['id'] }}" @if(in_array($v['id'], old('tag_ids', []))) checked="checked" @endif> &emsp;
                    @endforeach
                    <i class="fa fa-plus-square" style="font-size: 20px;cursor: pointer" data-toggle="modal" data-target="#yzh-tag-modal"></i>
                </td>
            </tr>
            <tr>
                <th>封面图</th>
    ...
    {{--添加标签--}}
    <div class="modal fade" id="yzh-tag-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <form class="form-inline" role="form">
                        <input class="form-control yzh-tag-name" type="text" placeholder="标签名">
...
@section('js')
    ...
    <script>
        ...
        $(function() {
            ...
            testEditor = editormd("yzh-content", {
                width     : "100%",
                height    : 720,
                toc       : true,
        ...
        $('.js-add-tag').click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    alert(123123);
                    var redioStr = response.name+'<input class="yzh-icheck" type="checkbox" name="tag_ids[]" value="'+response.id+'" checked="checked"> &emsp;';
                    $('.fa-plus-square').before(redioStr);
                    icheckInit();
                    $('#yzh-tag-modal').modal('hide');
                },
